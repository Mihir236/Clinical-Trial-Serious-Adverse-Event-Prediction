...
from dataset.utils import smiles_txt_to_lst, protocol2feature, icdcode_text_2_lst_of_lst, sentence2vec, Read_from_local
...
import re
from dataset.utils import Read_from_local
try:
    from dataset.utils import smiles_txt_to_lst
except ImportError:
    def smiles_txt_to_lst(smiles):
        return eval(smiles) if isinstance(smiles, str) else ["unknown"]
try:
    from dataset.dataset import Dose_dataset
except ImportError:
    raise ImportError("Dose_dataset not found in dataset.dataset. Check imports or define Dose_dataset.")
from models.mesh_encode import mesh_term2feature

def preprocess_icd_codes(icd_codes):
    range_mapping = {
        'C00-D48': 'C00',
        'R00-R99': 'R00',
    }
    return [range_mapping.get(code, re.split(r'-', code)[0] if '-' in code else code) for code in icd_codes]

def dose(phase):
    target = 'drug-dose-prediction'
    print(f"Calling Read_from_local with target={target}, phase={phase}")
    X_train, y_train, X_test, y_test = Read_from_local(target, phase)

    expected_column = 'dose'
    possible_columns = ['dose', 'dose_value', 'Avg', 'label']
    if expected_column not in y_train.columns:
        for col in possible_columns:
            if col in y_train.columns:
                print(f"Using '{col}' instead of '{expected_column}' for y_train")
                y_train_column = col
                break
        else:
            raise KeyError(f"'{expected_column}' not found in y_train. Available columns: {y_train.columns.tolist()}")
    else:
        y_train_column = expected_column

    if expected_column not in y_test.columns:
        for col in possible_columns:
            if col in y_test.columns:
                y_test_column = col
                break
        else:
            raise KeyError(f"'{expected_column}' not found in y_test. Available columns: {y_test.columns.tolist()}")
    else:
        y_test_column = expected_column

    y_train = y_train[y_train_column]
    y_test = y_test[y_test_column]

    if 'icdcode' in X_train.columns:
        X_train['icdcode'] = preprocess_icd_codes(X_train['icdcode'])
        X_test['icdcode'] = preprocess_icd_codes(X_test['icdcode'])
    ...
    def dose_collate_fn(x):
        ...
        label_vec = default_collate([np.array(i[1]) for i in x])
        smiles_lst = [smiles_txt_to_lst(i[2]) for i in x]
        mesh_lst = [mesh_term2feature(i[3][0]) if i[3] else np.zeros((1, 256)).astype(np.float32) for i in x]
        return [nctid_lst, label_vec, smiles_lst, mesh_lst]

    train_nctid_lst = X_train.index.tolist()
    train_label_lst = y_train.values.tolist()
    train_smiles_lst = X_train['smiless'].fillna('["unknown"]').tolist()
    train_mesh_lst = X_train[mesh_term].fillna('unknown').values.tolist() if mesh_term else [[]] * len(X_train)
    ...
    valid_label_lst = y_valid.tolist()
    valid_smiles_lst = X_valid['smiless'].fillna('["unknown"]').tolist()
    valid_mesh_lst = X_valid[mesh_term].fillna('unknown').values.tolist() if mesh_term else [[]] * len(X_valid)
    ...
    test_label_lst = y_test.tolist()
    test_smiles_lst = X_test['smiless'].fillna('["unknown"]').tolist()
    test_mesh_lst = X_test[mesh_term].fillna('unknown').values.tolist() if mesh_term else [[]] * len(X_test)
    ...
    return train_loader, valid_loader, test_loader, 0, 0
...
def dose_cls(phase):
    ...
    expected_column = 'Avg'
    possible_columns = ['Avg', 'dose_class', 'label', 'cls', 'dose_category']
    if expected_column not in y_train.columns:
        for col in possible_columns:
            if col in y_train.columns:
                print(f"Using '{col}' instead of '{expected_column}' for y_train")
    ...
    def dose_collate_fn(x):
        nctid_lst = [i[0] for i in x]
        label_vec = default_collate([i[1] for i in x])
        smiles_lst = [smiles_txt_to_lst(i[2]) for i in x]
        mesh_lst = [mesh_term2feature(i[3][0]) if i[3] else np.zeros((1, 256)).astype(np.float32) for i in x]
        return [nctid_lst, label_vec, smiles_lst, mesh_lst]
